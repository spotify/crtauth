...
def test_serialize_token_binary():
    ...
    assert buf == six.b(
        "t\x00\x00\x00Q]\x88\xaeQ]\x89\xda\x00\x00\x00\x03noa\x00"
    )


def test_deserialize_token_________rename_me():
    buf = six.b("t\x00\x00\x00Q]\x8b\x17Q]\x8bU\x00\x00\x00\x04test")
    token = protocol.Token.deserialize(buf)
    assert token.username == six.b('test')
    assert token.valid_from == 1365084951


def test_serialize_challenge():
    unique_data = six.b(
        '\x859\x9eHK\xc6\x83=\x0c,\xda\xf7K\x8e\xc3\xea}:$\xf8'
    )
    challenge = protocol.Challenge(
        unique_data=unique_data,
        valid_from=1365084334,
        valid_to=1367073504,
        fingerprint=six.b("\t\x02\xc8|\x83["),
        server_name=six.b("example.com"),
        username=six.b("user")
    )
    buf = protocol.Challenge.serialize(challenge)
    res = six.b(
        'c\x00\x00\x00\x859\x9eHK\xc6\x83=\x0c,\xda\xf7K\x8e\xc3\xea}:$'
...
def test_serialize_response():
    challenge = six.b(
        'v\x00\x00\x00O\x85q\x97\x03C\x17\x87\xbc\xb4\xa3\xeb1|\xd7'
    ...
    s = six.b(
        'r\x00\x00\x00\x00\x00\x01\x00g\xf2\xdbSMpha\n\xdb\xd0W\x08r\x90`F'
